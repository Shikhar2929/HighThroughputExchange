...
public class MatchingEngineTest {
    ...
    public void testBidLimitOrder_AddsBidSuccessfully() {
        ...
        Order bidOrder = new Order("TraderA", ticker,100, 10, Side.BID, Status.ACTIVE);
    ...
    public void testAskLimitOrder_AddsAskSuccessfully() {
        ...
        String ticker = "AAPL";
        engine.initializeTicker(ticker);
        engine.initializeUserTickerVolume("TraderB", ticker, 105);
        Order askOrder = new Order("TraderB", ticker, 105, 15, Side.ASK, Status.ACTIVE);
    ...
    public void testGetHighestBid_AfterMultipleBids() {
        ...
        engine.initializeUserBalance("TraderB", 10000);
        engine.bidLimitOrder("TraderA", new Order("TraderA", ticker, 100, 10, Side.BID, Status.ACTIVE));
        engine.bidLimitOrder("TraderB", new Order("TraderB", ticker, 105, 5, Side.BID, Status.ACTIVE));

        assertTrue(almostEqual(engine.getHighestBid(ticker), 105), "Highest bid should be 105");
    }
    ...
    public void testGetLowestAsk_AfterMultipleAsks() {
        ...
        engine.askLimitOrder("TraderA", new Order("TraderA", ticker, 110, 10, Side.ASK, Status.ACTIVE));
        engine.askLimitOrder("TraderB", new Order("TraderB", ticker, 105, 5, Side.ASK, Status.ACTIVE));
        System.out.println(engine.getLowestAsk(ticker));
        assertTrue(almostEqual(engine.getLowestAsk(ticker), 105), "Lowest ask should be 105");
    }
    ...
    public void testMatchingBidAndAskOrders() {
        ...
        engine.initializeUserTickerVolume("TraderB", ticker, 10);
        engine.bidLimitOrder("TraderA", new Order("TraderA", ticker, 105, 10, Side.BID, Status.ACTIVE));
        engine.askLimitOrder("TraderB", new Order("TraderB", ticker,105, 10, Side.ASK, Status.ACTIVE));

        assertTrue(almostEqual(engine.getHighestBid(ticker), 0), "Highest bid should be 0 after matching");
        System.out.println(engine.getLowestAsk(ticker));
    ...
    public void testInsertBid() {
        ...
        Map<Double, Deque<Order>> bids = engine.getBids(ticker);
        assertTrue(bids.containsKey(100), "Bids should contain the inserted order price");
        assertEquals(10, bids.get(100).peek().volume, "Bid volume should be 10");
    }
    ...
    public void testInsertAsk() {
        ...
        engine.initializeUserTickerVolume("TraderB", ticker, 10);
        Order askOrder = new Order("TraderB", ticker,100, 5, Side.ASK, Status.ACTIVE);

        long orderId = engine.askLimitOrder(askOrder.name, askOrder);
        assertTrue(orderId > 0, "Order ID should be valid and greater than 0");

        Map<Double, Deque<Order>> asks = engine.getAsks(ticker);
        assertTrue(asks.containsKey(100), "Asks should contain the inserted order price");
        assertEquals(5, asks.get(100).peek().volume, "Ask volume should be 5");
    }
    ...
    public void testFillOrderCompletely() {
        ...
        engine.initializeUserTickerVolume("TraderB", ticker, 0);
        Order askOrder = new Order("TraderA", ticker, 100, 5, Side.ASK, Status.ACTIVE);
        Order bidOrder = new Order("TraderB", ticker, 100, 5, Side.BID, Status.ACTIVE);
    ...
    public void testPartialFill() {
        ...
        engine.initializeUserTickerVolume("TraderB", ticker, 0);
        Order askOrder = new Order("TraderA", ticker, 100, 5, Side.ASK, Status.ACTIVE);
        Order bidOrder = new Order("TraderB", ticker, 100, 10, Side.BID, Status.ACTIVE);
        ...
        assertTrue(bids.containsKey(100), "Bids should contain the remaining bid at 100");
        assertEquals(5, bids.get(100).peek().volume, "Remaining bid volume should be 5 after partial fill");
    }
    ...
    public void testDifferentPricesNoMatch() {
        ...
        engine.initializeUserTickerVolume("TraderA", ticker, 10);
        engine.initializeUserTickerVolume("TraderB", ticker, 10);
        Order bid = new Order("TraderA", ticker, 95, 10, Side.BID, Status.ACTIVE);
        Order ask = new Order("TraderB", ticker, 105, 5, Side.ASK, Status.ACTIVE);
        ...
        Map<Double, Deque<Order>> bids = engine.getBids(ticker);
        Map<Double, Deque<Order>> asks = engine.getAsks(ticker);
        assertTrue(bids.containsKey(95), "Bids should contain the unmatched bid at 95");
        assertTrue(asks.containsKey(105), "Asks should contain the unmatched ask at 105");
    }
    @Test
    public void testCancelOrderValidBid() {
        ...
        engine.initializeTicker("AAPL");
        engine.initializeUserBalance("TraderA", 10000);
        Order bid = new Order("TraderA", ticker, 100, 10, Side.BID, Status.ACTIVE);
        ...
        //assertFalse(bids.containsKey(100), "Bids should not contain the canceled bid at 100");
    }
    ...
    public void testCancelOrderValidAsk() {
        ...
        engine.initializeUserBalance("TraderB", 10000);
        Order ask = new Order("TraderB", ticker, 150, 20, Side.ASK, Status.ACTIVE);
    ...
    void testBidPriceLevelsAfterAddingLimitOrder() {
        ...
        Order bidOrder1 = new Order("Trader1", ticker, 100, 5, Side.BID, Status.ACTIVE);
        ...
        assertEquals(5, bidLevels.get(0).volume);
    }
    @Test
    public void testAskPriceLevelsAfterAddingLimitOrder() {
        ...
        engine.initializeUserTickerVolume("Trader2", ticker, 3);
        Order askOrder1 = new Order("Trader2", ticker, 105, 3, Side.ASK, Status.ACTIVE);
        ...
        assertEquals(105, askLevels.get(0).price, "Ask price level should match the inserted order's price");
        assertEquals(3, askLevels.get(0).volume, "Ask volume at the level should match the inserted order's volume");
    }
    ...
    public void testPriceLevelsAfterPartialFill() {
        ...
        String ticker = "AAPL";
        engine.initializeTicker("AAPL");
        engine.initializeUserBalance("Trader1", 10000);
        engine.initializeUserBalance("Trader2", 10000);
        engine.initializeUserTickerVolume("Trader2", "AAPL", 100);
        engine.initializeUserTickerVolume("Trader1", ticker, 0);
        Order askOrder1 = new Order("Trader2", ticker, 100, 5, Side.ASK, Status.ACTIVE);
        Order bidOrder1 = new Order("Trader1", ticker, 100, 3, Side.BID, Status.ACTIVE);
        ...
        assertEquals(100, askLevels.get(0).price, "Ask price level should remain unchanged after partial fill");
        assertEquals(2, askLevels.get(0).volume, "Remaining ask volume should be correct after partial fill");
    ...
    public void testPriceLevelsWithMultipleOrdersAtSamePrice() {
        ...
        Order bidOrder1 = new Order("Trader1", ticker, 100, 3, Side.BID, Status.ACTIVE);
        Order bidOrder2 = new Order("Trader2", ticker,100, 2, Side.BID, Status.ACTIVE);
        ...
        assertEquals(100, bidLevels.get(0).price, "Bid price level should match the inserted orders' price");
        assertEquals(5, bidLevels.get(0).volume, "Bid volume should be aggregated across orders at the same price");

        Order askOrder1 = new Order("Trader3", ticker, 100, 4, Side.ASK, Status.ACTIVE);
        engine.askLimitOrder(askOrder1.name, askOrder1);
        ...
        assertEquals(100, bidLevels.get(0).price, "Bid price level should remain unchanged");
        assertEquals(1, bidLevels.get(0).volume, "Remaining bid volume should be correct after partial fill");
    ...
    void testMarketBuyOrderFullyFilled() {
        ...
        engine.initializeUserTickerVolume("Seller2", ticker, 100000);
        // Setup initial asks
        engine.askLimitOrder("Seller1", new Order("Seller1", ticker, 100, 200, Side.ASK, Status.ACTIVE));
        engine.askLimitOrder("Seller2", new Order("Seller2", ticker, 101, 400, Side.ASK, Status.ACTIVE));

        // Market Buy Order
        double filledVolume = engine.bidMarketOrder("Buyer1", ticker,500);

        // Assert filled volume
        assertEquals(500, filledVolume);
    ...
    void testMarketSellOrderFullyFilled() {
        ...
        // Setup initial bids
        engine.bidLimitOrder("Buyer1", new Order("Buyer1", ticker, 99, 150, Side.BID, Status.ACTIVE));
        engine.bidLimitOrder("Buyer2", new Order("Buyer2", ticker, 98, 200, Side.BID, Status.ACTIVE));

        // Market Sell Order
        double filledVolume = engine.askMarketOrder("Seller1", ticker, 300);

        // Assert filled volume
        assertEquals(300, filledVolume);

        // Assert remaining bids
        assertEquals(98, engine.getHighestBid(ticker));
        assertEquals(0, engine.getOrder("Buyer1", 1).volume);
        assertEquals(50, engine.getOrder("Buyer2", 2).volume);
    }
    ...
    void testMarketBuyOrderPartiallyFilled() {
        ...
        engine.initializeUserBalance("Buyer1", 100000);
        engine.initializeUserTickerVolume("Seller1", ticker, 500);
        engine.initializeUserTickerVolume("Buyer1", ticker, 0);
        // Setup initial asks
        engine.askLimitOrder("Seller1", new Order("Seller1", ticker, 100, 200, Side.ASK, Status.ACTIVE));
    ...
    void testMarketSellOrderPartiallyFilled() {
        MatchingEngine engine = new MatchingEngine();
        engine.initializeUserBalance("Buyer1", 30000);
        engine.initializeUserBalance("Seller1", 30000);
        String ticker = "AAPL";
        engine.initializeTicker("AAPL");
        engine.initializeUserTickerVolume("Buyer1", ticker, 0);
        engine.initializeUserTickerVolume("Seller1", ticker, 1000);

        // Setup initial bids
        engine.bidLimitOrder("Buyer1", new Order("Buyer1", ticker, 99, 300, Side.BID, Status.ACTIVE));

        // Market Sell Order
        double filledVolume = engine.askMarketOrder("Seller1", ticker, 500);
    ...
    void testMarketOrderCancelledDueToNoLiquidity() {
        ...
        double buyFilledVolume = engine.bidMarketOrder("Buyer1", ticker, 500);

        // Market Sell Order with no bids
        double sellFilledVolume = engine.askMarketOrder("Seller1", ticker, 300);

        // Assert filled volume
        assertEquals(0, buyFilledVolume);
        assertEquals(0, sellFilledVolume);
    }
    @Test
    void testUninitializedUser() {
        ...
        long orderId = engine.bidLimitOrder("Buyer1", new Order("Buyer1", ticker, 99, 150, Side.BID, Status.ACTIVE));
        assertEquals(orderId, -1); // -1 represents invalid user
        orderId = engine.askLimitOrder("RandomName", new Order("RandomName", ticker,5, 5, Side.ASK, Status.ACTIVE));
        assertEquals(orderId, -1);
        engine.initializeUserBalance("ActualUser", 1000);
        engine.initializeUserTickerVolume("ActualUser", ticker, 5);
        engine.askLimitOrder("ActualUser", new Order("ActualUser", ticker, 10, 5, Side.ASK, Status.ACTIVE));
        engine.bidLimitOrder("ActualUser", new Order("ActualUser", ticker, 8, 5, Side.BID, Status.ACTIVE));
        double orderFilledVolume = engine.bidMarketOrder("Buyer1", ticker, 1000);
        assertEquals(orderFilledVolume, 0);
    ...
    void testUserListLimit() {
        ...
        Order bidOrder = new Order("Trader1", ticker, 100, 5, Side.BID, Status.ACTIVE);
        long orderId = engine.bidLimitOrder(bidOrder.name, bidOrder);
        assertEquals(engine.getUserBalance("Trader1"), 1000);
        Order askOrder = new Order("Trader2", ticker, 99, 5, Side.ASK, Status.ACTIVE);
        ...
        assertEquals(engine.getUserBalance("Trader2"), 1500);
        //
        Order askOrder2 = new Order("Trader1", ticker, 100, 5, Side.ASK, Status.ACTIVE);
        engine.askLimitOrder("Trader1", askOrder2);
        assertEquals(engine.getUserBalance("Trader1"), 500);
        assertEquals(engine.getUserBalance("Trader2"), 1500);
        Order bidOrder2 =  new Order("Trader2", ticker, 102, 5, Side.BID, Status.ACTIVE);
    ...
    void testUserListMarket() {
        ...
        Order bidOrder = new Order("Trader1", ticker, 100, 10, Side.BID, Status.ACTIVE);
        long orderId = engine.bidLimitOrder(bidOrder.name, bidOrder);
        engine.askMarketOrder("Trader2", ticker, 5);
        assertEquals(engine.getUserBalance("Trader1"), 500);
        assertEquals(engine.getUserBalance("Trader2"), 1000);
        Order askOrder =  new Order("Trader1", ticker, 101, 10, Side.ASK, Status.ACTIVE);
        orderId = engine.askLimitOrder(askOrder.name, askOrder);
        double volume = engine.bidMarketOrder("Trader2", ticker, 10);
        assertEquals(1000 / 101, volume);
    ...
    void testBidLimitOrderBalanceUpdates() {
        ...
        Order bidOrder = new Order("Trader1", ticker, 50, 10, Side.BID, Status.ACTIVE);
        long orderId = engine.bidLimitOrder(bidOrder.name, bidOrder);

        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should be reduced by bid order value");
        assertEquals(0, engine.getTickerBalance("Trader1", ticker), "Ticker balance should remain unchanged for bid order");

        // Remove the order and verify balance restoration
        engine.removeOrder("Trader1", orderId);
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should be restored after bid order cancellation");
    }
    ...
    void testAskLimitOrderBalanceUpdates() {
        ...
        Order askOrder = new Order("Trader1", ticker, 50, 10, Side.ASK, Status.ACTIVE);
        long orderId = engine.askLimitOrder(askOrder.name, askOrder);

        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should remain unchanged for ask order");
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker balance should be reduced by ask order volume");

        // Remove the order and verify ticker balance restoration
        engine.removeOrder("Trader1", orderId);
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker balance should be restored after ask order cancellation");
    }
    ...
    void testBidMarketOrderBalanceUpdates() {
        ...
        engine.askLimitOrder("Seller1", new Order("Seller1", ticker, 50, 10, Side.ASK, Status.ACTIVE));

        // Buyer places a market order
        double volumeFilled = engine.bidMarketOrder("Buyer1", ticker, 10);

        assertEquals(10, volumeFilled, "Market order should fully match the ask order volume");
        assertEquals(0, engine.getUserBalance("Buyer1"), "Buyer's balance should be fully consumed");
        assertEquals(10, engine.getTickerBalance("Buyer1", ticker), "Buyer's ticker balance should increase by matched volume");
        assertEquals(1000, engine.getUserBalance("Seller1"), "Seller's balance should increase by the transaction value");
        assertEquals(0, engine.getTickerBalance("Seller1", ticker), "Seller's ticker balance should be reduced by the sold volume");
    }
    ...
    void testRemoveAllOrdersRestoresBalances() {
        ...
        engine.bidLimitOrder("Trader1", new Order("Trader1", ticker, 50, 10, Side.BID, Status.ACTIVE));
        engine.askLimitOrder("Trader1", new Order("Trader1", ticker, 60, 5, Side.ASK, Status.ACTIVE));

        // Remove all orders and verify balances
        engine.removeAll("Trader1");
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should be restored after removing all orders");
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker balance should be restored after removing all orders");
    }
    @Test
    void testRemoveOrderRestoresAllProperties() {
        ...
        // Place an order
        Order bidOrder = new Order("Trader1", ticker, 100, 1, Side.BID, Status.ACTIVE);
        long orderId = engine.bidLimitOrder(bidOrder.name, bidOrder);

        // Verify initial state
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should reflect bid reservation");
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker volume should remain unchanged for bid orders");
        ...
        // Verify removal
        assertTrue(removed, "Order should be successfully removed");
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should be restored after removing bid order");
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker balance should remain unchanged after removing bid order");
    ...
    void testRemoveAllRestoresAllProperties() {
        ...
        // Verify initial state
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should reflect bid reservation");
        assertEquals(15, engine.getTickerBalance("Trader1", ticker), "Ticker volume should reflect ask reservation");

        // Remove all orders
        engine.removeAll("Trader1");

        // Verify all properties are restored
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should be fully restored after removing all orders");
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker balance should be fully restored after removing all orders");
        List<PriceLevel> bidLevels = engine.getBidPriceLevels(ticker);
        List<PriceLevel> askLevels = engine.getAskPriceLevels(ticker);
    ...
    void testRemoveOrderForAskRestoresAllProperties() {
        ...
        // Place an ask order
        Order askOrder = new Order("Trader1", ticker, 100, 10, Side.ASK, Status.ACTIVE);
        long orderId = engine.askLimitOrder(askOrder.name, askOrder);

        // Verify initial state
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should remain unchanged for ask orders");
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker balance should reflect ask reservation");
        ...
        assertEquals(1000, engine.getUserBalance("Trader1"), "User balance should remain unchanged after removing ask order");
        assertEquals(20, engine.getTickerBalance("Trader1", ticker), "Ticker balance should be fully restored after removing ask order");
    ...
    public void testAddTradeAndCheckVolumeSummation() {
        ...
        assertEquals(15, trade.getVolume(), "Trade volume should be the sum of updates");
    ...
    public void testLimitOrderTradeLogging() {
        ...
        String ticker = "GOOG";
        engine.initializeTicker(ticker);
        engine.initializeUserBalance("TraderC", 2000);
        engine.initializeUserTickerVolume("TraderC", ticker, 50);

        // Place a bid limit order and an ask limit order that should match
        engine.bidLimitOrder("TraderC", new Order("TraderC", ticker, 150, 5, Side.BID, Status.ACTIVE));
        engine.askLimitOrder("TraderC", new Order("TraderC", ticker, 150, 5, Side.ASK, Status.ACTIVE));
    ...
    public void testMarketOrderTradeLogging() {
        ...
        String ticker = "GOOG";
        engine.initializeTicker(ticker);
        engine.initializeUserBalance("TraderD", 5000);
        engine.initializeUserTickerVolume("TraderD", ticker, 100);

        // Set up limit orders to be matched by a market order
        engine.askLimitOrder("TraderD", new Order("TraderD", ticker, 200, 20, Side.ASK, Status.ACTIVE));
        engine.askLimitOrder("TraderD", new Order("TraderD", ticker, 205, 20, Side.ASK, Status.ACTIVE));
        List<PriceChange> trades = RecentTrades.getRecentTrades();
    ...
    public void testCancelOrderTradeLogging() {
        ...
        String ticker = "GOOG";
        engine.initializeTicker(ticker);
        engine.initializeUserBalance("TraderE", 10000);
        engine.initializeUserTickerVolume("TraderE", ticker, 50);

        // Place and cancel a bid limit order
        long orderId = engine.bidLimitOrder("TraderE", new Order("TraderE", ticker, 250, 10, Side.BID, Status.ACTIVE));
    ...
    private static final double POSITION_LIMIT = 100;
    ...
    public void infiniteTestTradingWithSelf() {
        ...
        Order sellOrder = new Order(user, ticker, 150, 50, Side.ASK, Status.ACTIVE); // Short additional 50 to reach -100
