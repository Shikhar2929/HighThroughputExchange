...
class WebsocketOnboardingGapReproTest {
    ...
    void updateBetweenSnapshotAndSubscribe_canBeLost() throws Exception {
        ...
        matchingEngine.bidLimitOrder(username, new Order(username, "A", priceBeforeSubscribe, 1, Side.BID, Status.ACTIVE));

        // Force the websocket broadcaster to flush-and-clear changes while nobody is
        // subscribed.
        // This simulates: update sent while client is still onboarding / before
        // subscribe.
        ...
        try {
            ...
            socketController.sendRecentTrades();
            assertNotNull(awaitAnySocketContent(receivedContents, 2, TimeUnit.SECONDS),
                    "Expected to receive at least one websocket message after subscribing");
            receivedContents.clear();

            // Step 4: Another update happens AFTER subscribe; client should receive this
            // one.
            int priceAfterSubscribe = 101;
            matchingEngine.bidLimitOrder(username, new Order(username, "A", priceAfterSubscribe, 1, Side.BID, Status.ACTIVE));
            socketController.sendRecentTrades();

            String payload = awaitTradePayloadContainingPrice(receivedContents, priceAfterSubscribe, 3, TimeUnit.SECONDS);
            assertNotNull(payload, "Expected to receive a websocket update containing the post-subscribe price");

            // Assert: client did NOT receive the pre-subscribe update (this is the bug)
            assertFalse(payload.contains("\"price\" : " + priceBeforeSubscribe),
            ...
            assertFalse(anyPayloadContainsPrice(receivedContents, priceBeforeSubscribe, 800, TimeUnit.MILLISECONDS),
    ...
    private WebSocket connectAndSubscribe(String username, String sessionToken, BlockingQueue<String> receivedContents) throws Exception {
        String wsUrl = "ws://localhost:" + port + "/exchange-socket?Session-ID=" + sessionToken + "&Username=" + username;
        ...
        WebSocket socket = HttpClient.newHttpClient().newWebSocketBuilder().buildAsync(URI.create(wsUrl), listener).get(5, TimeUnit.SECONDS);
        ...
        // Start a lightweight parser that converts MESSAGE frames into extracted
    ...
    private void awaitConnectedFrame(BlockingQueue<String> rawFrames, long timeout, TimeUnit unit) throws InterruptedException {
    ...
    private String awaitTradePayloadContainingPrice(BlockingQueue<String> receivedContents, int requiredPrice, long timeout, TimeUnit unit)
        ...
        while (System.nanoTime() < deadlineNanos) {
            ...
            // SocketController sometimes emits "No recent trades"; skip until we get a JSON
            // array
            // payload.
    ...
    private String awaitAnySocketContent(BlockingQueue<String> receivedContents, long timeout, TimeUnit unit) throws InterruptedException {
        return receivedContents.poll(timeout, unit);
    }

    private boolean anyPayloadContainsPrice(BlockingQueue<String> receivedContents, int forbiddenPrice, long timeout, TimeUnit unit)
            throws InterruptedException {
        long deadlineNanos = System.nanoTime() + unit.toNanos(timeout);
        while (System.nanoTime() < deadlineNanos) {
            String content = receivedContents.poll(200, TimeUnit.MILLISECONDS);
