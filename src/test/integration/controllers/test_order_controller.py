...
def test_limit_order_success(client: ExchangeClient, unique_username) -> None:
    ...
    api_key = client.admin_add_user(
        username=username, name="CI Trader", email=f"{username}@example.com"
    )
...
def test_remove_all_succeeds_after_order(
    ...
    api_key = client.admin_add_user(
        username=username, name="CI RemoveAll", email=f"{username}@example.com"
    )
    session = client.buildup(username, api_key)
...
def test_remove_by_order_id(client: ExchangeClient, unique_username) -> None:
    ...
    api_key = client.admin_add_user(
        username=username, name="CI Remove", email=f"{username}@example.com"
    )
    session = client.buildup(username, api_key)
...
def test_market_order_matches_against_limit(
    client: ExchangeClient, unique_username
) -> None:
    client.admin_set_state(1)
...
def test_bot_flow_can_place_order_and_remove_all(
    client: ExchangeClient, unique_username
) -> None:
    client.admin_set_state(1)
...
def test_bot_market_order_success(client: ExchangeClient, unique_username) -> None:
    ...
    # Seed the book with opposite-side liquidity so the market order fills.
    seller = unique_username("mkt_seller")
    seller_key = client.admin_add_user(
        username=seller, name="CI Seller", email=f"{seller}@example.com"
    )
    seller_session = client.buildup(seller, seller_key)
