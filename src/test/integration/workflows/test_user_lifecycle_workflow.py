...
def test_user_lifecycle_create_trade_remove_details_teardown(client: ExchangeClient, unique_username) -> None:
    ...
    api_key = client.admin_add_user(username=username, name="WF User", email=f"{username}@example.com")
...
def test_two_users_trade_then_remove_all(client: ExchangeClient, unique_username) -> None:
