...
def remove_all(session_token):
    ...
    req = urllib.request.Request(
        URL + "/remove_all", data=json.dumps(form_data).encode("utf-8"), method="POST"
    )
    req.add_header("Content-Type", "application/json")
    response = urllib.request.urlopen(req).read().decode("utf-8")
...
if __name__ == "__main__":
    ...
    api_key = (
        user_data["apiKey"] if "apiKey" in user_data else exit("Failed to create user.")
    )

    session_data = buildup(username, api_key)
    session_token = (
        session_data["sessionToken"]
        if "sessionToken" in session_data
        else exit("Failed to buildup.")
    )
