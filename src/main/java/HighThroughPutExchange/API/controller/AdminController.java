...
public class AdminController {
    ...
    public ResponseEntity<AddUserResponse> addUser(@Valid @RequestBody AddUserRequest form) {
        ...
        return new ResponseEntity<>(new AddUserResponse(Message.SUCCESS.toString(), user.getApiKey(), user.getApiKey2()), HttpStatus.OK);
    }
    ...
    public ResponseEntity<AddUserResponse> addBot(@Valid @RequestBody AddBotRequest form) {
        ...
        return new ResponseEntity<>(new AddUserResponse(Message.SUCCESS.toString(), key), HttpStatus.OK);
    }
    ...
    public ResponseEntity<LeaderboardResponse> leaderboard(@Valid @RequestBody LeaderboardRequest form) {
        ...
        return new ResponseEntity<>(new LeaderboardResponse(Message.SUCCESS.toString(), data), HttpStatus.OK);
    }
    ...
    public ResponseEntity<SetStateResponse> setState(@Valid @RequestBody SetStateRequest form) {
        if (!authService.authenticateAdmin(form)) {
            return new ResponseEntity<>(new SetStateResponse(Message.AUTHENTICATION_FAILED.toString(), 0), HttpStatus.UNAUTHORIZED);
        }

        if (form.getTargetState() >= State.values().length || form.getTargetState() < 0) {
            return new ResponseEntity<>(new SetStateResponse(Message.BAD_INPUT.toString(), 0), HttpStatus.BAD_REQUEST);
        }

        int newStateOrdinal = adminService.applyState(form.getTargetState());
        return new ResponseEntity<>(new SetStateResponse(Message.SUCCESS.toString(), newStateOrdinal), HttpStatus.OK);
    }
